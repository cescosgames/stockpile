...
<p align="center">
  <a href="https://stockpile-4em.pages.dev"><strong>Web Demo: stockpile-4em.pages.dev</strong></a>
</p>
...
- **Dashboard** — Stat cards for animal count, low stock, and AM/PM task progress. Animal health summary, feed inventory bars, and a Farm Notes section for freeform dated entries with optional author attribution. Notes can be edited inline after creation.
- **Settings** — Farm name and timezone. Export/import full farm data (animals, feed inventory, and all tasks) as a JSON backup file. Wipe all data option. Sync mode toggle (Local only / Local network) with Pi URL configuration.
...
- **PWA** — Install to home screen on any device (iOS, Android, desktop). Zoom locked, safe-area insets applied, oversized touch targets on action buttons, and a responsive header date (compact on mobile, full on desktop) — feels native rather than web. On mobile, navigation moves to a fixed bottom tab bar (iOS convention) with a sliding accent indicator; on desktop, tabs stay at the top.
...
electron-builder builds for the OS it runs on — run the build on whichever platform you're targeting:

```bash
npm run electron:build
# macOS output:  dist-electron/Stockpile-<version>.dmg
# Windows output: dist-electron/Stockpile Setup <version>.exe
```

The packaged app stores data in `~/Library/Application Support/Stockpile/` (macOS) or `%APPDATA%\Stockpile\` (Windows).

> Note: builds are unsigned by default. macOS will warn "unidentified developer" — right-click → Open to bypass for personal use. Windows may show a SmartScreen warning. Public distribution requires an Apple Developer certificate + notarization (macOS) or a code-signing certificate (Windows).
...
| `public/pwa-512.icns` / `public/icon.ico` | Desktop icons (macOS / Windows) — regenerate from your PNG (`iconutil` on macOS, ImageMagick or online converters on other platforms) |
...
- **New features** — ideas welcome, open an issue to discuss. See the note on offline sync below.
- **Offline sync** — not yet implemented. The farm's WiFi doesn't cover the full property; farmhands working in the fields can't reach the Pi. The plan is to queue writes locally when offline and merge on reconnect. This is deliberately deferred until the Pi is live and real usage patterns are understood — the merge complexity (checkedState union, soft-deletes, feed qty reconciliation) isn't worth tackling speculatively. Current behaviour: reads work from local cache, writes are blocked with an Offline banner until connection is restored.
